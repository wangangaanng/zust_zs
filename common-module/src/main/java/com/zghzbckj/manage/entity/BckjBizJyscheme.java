/**
 * Copyright &copy; 2012-2016 <a href="https://github.com/thinkgem/jeesite">JeeSite</a> All rights reserved.
 */
package com.zghzbckj.manage.entity;


import com.zghzbckj.base.entity.DataWithExpEntity;


import java.util.Date;

/**
 * 就业方案Entity
 * @author wangangaanng
 * @version 2019-09-30
 */
public class BckjBizJyscheme extends DataWithExpEntity<BckjBizJyscheme> {
	
	private static final long serialVersionUID = 1L;
	private String yhRefOwid;
	private String xsxh;
	private String xm;
	private String xxmc;
	private String byqx;
	private Integer sfzydk;
	private String yrdwmc;
	private String yrdwdm;
	private String yrdwxzmc;
	private String dwhylbmc;
	private String dwszdmc;
	private String dwlxr;
	private String dwdh;
	private String gzzwlbmc;
	private String bdzqflbmc;
	private String bdzqwdwmc;
	private String bdzqwszdmc;
	private Date bdkssj;
	private Date bdjssj;
	private Integer sfdydwbdz;
	private String datddw;
	private String bdzbz;
	private String datdxxdz;
	private String datddh;
	private String hkqydz;
	private String bzone;
	private String bztwo;
	private String bzthree;

	public String getYhRefOwid() {
		return yhRefOwid;
	}

	public void setYhRefOwid(String yhRefOwid) {
		this.yhRefOwid = yhRefOwid;
	}

	public String getXsxh() {
		return xsxh;
	}

	public void setXsxh(String xsxh) {
		this.xsxh = xsxh;
	}

	public String getXm() {
		return xm;
	}

	public void setXm(String xm) {
		this.xm = xm;
	}

	public String getXxmc() {
		return xxmc;
	}

	public void setXxmc(String xxmc) {
		this.xxmc = xxmc;
	}

	public String getByqx() {
		return byqx;
	}

	public void setByqx(String byqx) {
		this.byqx = byqx;
	}

	public Integer getSfzydk() {
		return sfzydk;
	}

	public void setSfzydk(Integer sfzydk) {
		this.sfzydk = sfzydk;
	}

	public String getYrdwmc() {
		return yrdwmc;
	}

	public void setYrdwmc(String yrdwmc) {
		this.yrdwmc = yrdwmc;
	}

	public String getYrdwdm() {
		return yrdwdm;
	}

	public void setYrdwdm(String yrdwdm) {
		this.yrdwdm = yrdwdm;
	}

	public String getYrdwxzmc() {
		return yrdwxzmc;
	}

	public void setYrdwxzmc(String yrdwxzmc) {
		this.yrdwxzmc = yrdwxzmc;
	}

	public String getDwhylbmc() {
		return dwhylbmc;
	}

	public void setDwhylbmc(String dwhylbmc) {
		this.dwhylbmc = dwhylbmc;
	}

	public String getDwszdmc() {
		return dwszdmc;
	}

	public void setDwszdmc(String dwszdmc) {
		this.dwszdmc = dwszdmc;
	}

	public String getDwlxr() {
		return dwlxr;
	}

	public void setDwlxr(String dwlxr) {
		this.dwlxr = dwlxr;
	}

	public String getDwdh() {
		return dwdh;
	}

	public void setDwdh(String dwdh) {
		this.dwdh = dwdh;
	}

	public String getGzzwlbmc() {
		return gzzwlbmc;
	}

	public void setGzzwlbmc(String gzzwlbmc) {
		this.gzzwlbmc = gzzwlbmc;
	}

	public String getBdzqflbmc() {
		return bdzqflbmc;
	}

	public void setBdzqflbmc(String bdzqflbmc) {
		this.bdzqflbmc = bdzqflbmc;
	}

	public String getBdzqwdwmc() {
		return bdzqwdwmc;
	}

	public void setBdzqwdwmc(String bdzqwdwmc) {
		this.bdzqwdwmc = bdzqwdwmc;
	}

	public String getBdzqwszdmc() {
		return bdzqwszdmc;
	}

	public void setBdzqwszdmc(String bdzqwszdmc) {
		this.bdzqwszdmc = bdzqwszdmc;
	}

	public Date getBdkssj() {
		return bdkssj;
	}

	public void setBdkssj(Date bdkssj) {
		this.bdkssj = bdkssj;
	}

	public Date getBdjssj() {
		return bdjssj;
	}

	public void setBdjssj(Date bdjssj) {
		this.bdjssj = bdjssj;
	}

	public Integer getSfdydwbdz() {
		return sfdydwbdz;
	}

	public void setSfdydwbdz(Integer sfdydwbdz) {
		this.sfdydwbdz = sfdydwbdz;
	}

	public String getDatddw() {
		return datddw;
	}

	public void setDatddw(String datddw) {
		this.datddw = datddw;
	}

	public String getBdzbz() {
		return bdzbz;
	}

	public void setBdzbz(String bdzbz) {
		this.bdzbz = bdzbz;
	}

	public String getDatdxxdz() {
		return datdxxdz;
	}

	public void setDatdxxdz(String datdxxdz) {
		this.datdxxdz = datdxxdz;
	}

	public String getDatddh() {
		return datddh;
	}

	public void setDatddh(String datddh) {
		this.datddh = datddh;
	}

	public String getHkqydz() {
		return hkqydz;
	}

	public void setHkqydz(String hkqydz) {
		this.hkqydz = hkqydz;
	}

	public String getBzone() {
		return bzone;
	}

	public void setBzone(String bzone) {
		this.bzone = bzone;
	}

	public String getBztwo() {
		return bztwo;
	}

	public void setBztwo(String bztwo) {
		this.bztwo = bztwo;
	}

	public String getBzthree() {
		return bzthree;
	}

	public void setBzthree(String bzthree) {
		this.bzthree = bzthree;
	}
}