 ...
 * @author xby
 */
public class ZyActioin implements ComponentListinerSer {
    private static final String AREA_URL = "web/zustswyt/bckjBizBkzy/listByZy";

    @Override
    public void doAction(BaseWindow baseWindow, Event event, PageControlVO pageControlVO) {

        //学科类别
        BaseListbox provinceCode = (BaseListbox) baseWindow.getFellowIfAny("conditionGrid_bklb");
        //报考类别
        BaseListbox cityCode = (BaseListbox) baseWindow.getFellow("conditionGrid_xzzymc");
        String label = "";
        String value = "";
        /**初始化数据*/
        try {
