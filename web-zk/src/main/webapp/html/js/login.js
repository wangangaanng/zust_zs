            ...
            if (data.bean.length > 0) {//第一级
                $.each(data.bean, function (k, p) {
                    ...
                    if (index == 5 || index == 6) {
                        ...
                        if (k == 0) {
                            str = '<a class="wrap-list" path="" owid="">请选择</a><ul class="select-option"><li owid = "" path="">请选择</li>';
                        }
                        var cj2 = p.CC + 1;
                        str += '<li path=' + p.PATH + ' owid = ' + p.OWID + ' name= ' + p.NAME + ' index=' + index + ' cc=' + cj2 + ' >' + p.NAME + '</li>';
                        if (p.children && p.children.length > 0) {//第二级
                            $.each(p.children, function (x, y) {
                                if (y.children && y.children.length > 0) {//第三级
                                    DetailArr.set(y.NAME, y.children);
                                } else {
                                    if ($("#partlist3" + index).html() == "") {
                                        $("#partlist3" + index).html(nullThisTip);
                                    }
                                }
                            });
                        } else {
                            if ($("#partlist2" + index).html() == "") {
                                $("#partlist2" + index).html(nullThisTip);
                            }
                        }
                    }
                });
...
//下拉框选择后隐藏下拉框
$(document).on("click", ".select-overlay li li", function () {
